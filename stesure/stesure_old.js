...
function gantt(data) {
  ...
  // create first the titles, so to get the left margin according to the longest one
  title = groupsSvg.append('g').classed('gantt-titles', true).selectAll('.title').data(groups, d => {
    return d.key
  })
  title.exit().remove()
  title = title.enter().append('text')
    ...
    .merge(title)
    .each(function(d, i) {
      let thisWidth = d3.select(this).node().getBBox().width + 14
      margin.left = d3.max([(margin.left), thisWidth])
      console.log(margin.left, thisWidth)
    })
    .attr('transform', d => {
  ...
  group = groupsSvg.append('g').classed('gantt-lines', true).selectAll('.group').data(groups, d => {
    return d.key
  })
  group.exit().remove()
  group = group.enter().append('g')
    ...
    .attr('transform', d => {
      return `translate(0,${groupScale(d.key)})`;
    })
    .merge(group)
  ...
  groupTriangle = groupTriangle.enter().append('path')
    ...
    .attr('transform', `translate(${margin.left + 40 },0) rotate(90)`)
    .merge(groupTriangle)
    .on('click', function(d) {
  ...
  function collapse() {
    ...
    d3.selectAll('.triangle').transition().duration(500).attr('transform', `translate(${margin.left + 40 },0) rotate(90)`)
  ...
  function expand(triangle, data) {
    ...
    d3.select(triangle).transition().duration(500).attr('transform', `translate(${margin.left + 40 },0) rotate(180)`)
    ...
    d3.selectAll('#gantt g.group').each(function(d, i) {
      ...
      if (i == selectedIndex) {
        ...
        subGroup = subGroup.enter().append('g')
          ...
          .attr('transform', d => {
            return `translate(0,${ data.value.indexOf(d)*25 + yPosition + 5 + subHeight })`
          })
          .merge(subGroup)
      ...
      if (selectedIndex > -1 && i > selectedIndex) {
        d3.select(this).transition()
          ...
          // .delay(i * 25)
          .attr('transform', d => {
            return `translate(0,${groupScale(d.key) + data.value.length*subHeight})`;
          })
      } else {
        d3.select(this).transition()
          .duration(500)
